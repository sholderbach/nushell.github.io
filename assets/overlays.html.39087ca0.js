...
───┬──────
 <span class="token number">0</span> │ zero
───┴──────
</code></pre></div>`,3),y=a("code",null,"overlay hide",-1),z=s(`<h2 id="覆层是可记录的" tabindex="-1"><a class="header-anchor" href="#覆层是可记录的" aria-hidden="true">#</a> 覆层是可记录的</h2><p>任何新的定义（命令、别名、环境变量）都会被记录到最后一个活动的覆层：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token punctuation">(</span>zero<span class="token punctuation">)</span><span class="token operator">&gt;</span> overlay use spam
...
eggs
</code></pre></div><p>覆层会记住你所添加的内容，即使你删除了它们，也会储存这些信息。 这可以让你在不同的上下文中反复切换。</p>`,8),x={class:"custom-container tip"},q=s(`<p class="custom-container-title">TIP</p><p>有时，在添加一个覆层后，你可能不希望自定义对象被添加到其中。 解决的办法是创建一个新的空的覆层，只用来记录自定义的变化：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token punctuation">(</span>zero<span class="token punctuation">)</span><span class="token operator">&gt;</span> overlay use spam
...
</code></pre></div><p><code>eggs</code> 命令被添加到 <code>scratchpad</code> 中，同时保持 <code>spam</code> 不变。</p>`,4),C=a("em",null,"0.64 版本新增：",-1),N=a("code",null,"overlay new",-1),w=s(`<div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token punctuation">(</span>zero<span class="token punctuation">)</span><span class="token operator">&gt;</span> overlay use spam
...
───┴──────
</code></pre></div><p>现在，<code>zero</code> 覆层具有优先权。</p>`,8);function V(A,E){const o=u("RouterLink");return c(),l("div",null,[i,d,a("p",null,[a("em",null,[n("注意：要了解覆层，请确保先查看 "),t(o,{to:"/zh-CN/book/modules.html"},{default:e(()=>[n("模块")]),_:1}),n("，因为覆层是建立在模块之上的。")])]),k,a("p",null,[n("首先，Nushell 有一个默认的覆层，叫做 "),g,n("。 你可以用 "),t(o,{to:"/book/commands/overlay_list.html"},{default:e(()=>[h]),_:1}),n(" 命令检查哪些覆层是活动的。 你应该可以看到默认的覆层显示在那里。")]),m,a("p",null,[n("要创建覆层，请调用 "),t(o,{to:"/book/commands/overlay_add.html"},{default:e(()=>[v]),_:1}),n("。")]),_,a("p",null,[n("如果你不再需要叠加定义，请调用 "),t(o,{to:"/book/commands/overlay_remove.html"},{default:e(()=>[b]),_:1}),n("：")]),f,a("p",null,[n("此外，"),t(o,{to:"/book/commands/overlay_remove.html"},{default:e(()=>[y]),_:1}),n(" 在没有参数的情况下，将删除最后一个活动的覆层。")]),z,a("div",x,[q,a("p",null,[C,n(" 为了让上述步骤不那么冗长，你可以使用 "),t(o,{to:"/book/commands/overlay_new.html"},{default:e(()=>[N]),_:1}),n(" 命令：")]),w]),B])}const L=p(r,[["render",V],["__file","overlays.html.vue"]]);export{L as default};
